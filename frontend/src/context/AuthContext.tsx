...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = useCallback((userData: User) => {
    setUser(userData);
    localStorage.setItem('user', JSON.stringify(userData)); // Guardamos el objeto usuario completo
    localStorage.setItem('userEmail', userData.email); // Guardamos solo el email también
  }, []);
  ...
  const logout = useCallback(() => {
    ...
    localStorage.removeItem('accessToken');
    localStorage.removeItem('refreshToken');
    localStorage.removeItem('userEmail'); // Limpiar el email también
  }, []);
